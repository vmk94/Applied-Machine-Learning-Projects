...
#DATASET 3 ACCURACY TEST

#Fetch data from source
Data3 = np.array(np.genfromtxt('breast_cancer_winsconsin.csv',delimiter=',', dtype="|U50"))
#Read dataset number of rows and columns
Data3 = np.delete(Data3,0,1)
Data3_Rows, Data3_Columns = Data3.shape
#Target Binary Encoding
Target_Binary = np.zeros((Data3_Rows))
Target, counts = np.unique(Data3[:,-1], return_counts =True)
Target_Binary [np.where(Data3[:,-1] == Target[0])] = 1
#Delete Target
Data3 = Data3[:,:-1]
Data3_Rows, Data3_Columns = Data3.shape

    
for x in range(Data3_Columns):
    Mode = modevalue(Data3[:,x])
    Data3[np.where(Data3[:,x]== "?"),x] = Mode.astype('int')

Data3 = np.asarray(Data3).astype('int')
...
Accuracy_mean_NB_DS3 = np.mean(Accuracy_vector_NB)

print("Average Accuracy")
print(Accuracy_mean_NB_DS3)
